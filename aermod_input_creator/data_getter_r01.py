...
#--------------------------------------------------------------------------------------------------
x = '2021-03-10' #datetime.datetime.now() RETIRAR A DATA ENTRE PARÊNTESES E COLOCAR A PARTE COMENTADA
...
x = datetime.datetime.strptime(x, '%Y-%m-%d') #APAGAR A LINHA NA HORA DE IMPLEMENTAR
x = x.timetuple()
ano = x.tm_year
mes = x.tm_mon
dia = x.tm_mday
hora = x.tm_hour
DATA_HORA_REF = ("'" + str(ano) + '-' + str(mes) + '-' + str(dia) + "'")
#--------------------------------------------------------------------------------------------------
...
def dados_sodar(DATA_HORA_REF_SODAR):
	...
	try:
			...
			WHERE DATE(date) = """ + DATA_HORA_REF_SODAR + """ AND ((z BETWEEN 40 AND 350)
			OR (z = 400 OR z = 450 OR z = 500 OR z = 550 OR z = 600))
...
def dados_radiossonda(DATA_HORA_REF_SONDAGEM):
	#CONECTANDO AO BANCO DE DADOS
		...
		try:
				...
				WHERE DATE(date) = """ + DATA_HORA_REF_SONDAGEM + """ AND (baro = '1000' OR baro = '975' OR baro = '950')
				ORDER BY date ASC;"""
...
def dados_ems_ternium(DATA_HORA_REF_EMS_TERNIUM):
	#CONECTANDO AO BANCO DE DADOS
		...
		try:
			...
			WHERE DATE(date) = """ + DATA_HORA_REF_EMS_TERNIUM + """AND sms_llwas_id = 1 ORDER BY date ASC;"""
...
aux_ems = dados_ems_ternium(DATA_HORA_REF)
#--------------------------------------------------------------------------------------------------
...
#--------------------------------------------------------------------------------------------------
#BUSCANDO A ULTIMA MEDIÇÃO FEITA PELO SODAR
ultima_hora_medida  = datas_lidas[-1]
quant_h = ultima_hora_medida.hour
#ELA VAI DETERMINAR A QUANTIDADE DE HORAS NECESSÁRIAS PARA A FORMAÇÃO DO ARQUIVO
#--------------------------------------------------------------------------------------------------



#--------------------------------------------------------------------------------------------------
#ADICIONANDO AS DATAS NECESSÁRIAS PARA FORMAÇÃO DO ARQUIVO sodar.dat A UM ARRAY AUXILIAR - REFERENTE A SODAR
data_need = []
for b in range(0,quant_h+1): #'+1 TEM O OBJETIVO DE PEGAR ATÉ A AHORA ZERO'
	data_hora_inicial = ultima_hora_medida - datetime.timedelta(hours=b)
	for c in range(0,z):
